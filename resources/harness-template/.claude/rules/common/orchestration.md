...
## 직접 vs Workflow vs 팀 판정표 (v0.18.1)

| 유저 요청 패턴 | 기본 처리 | 격상 조건 |
|---------------|----------|----------|
| 버그 fix / 작은 수정 / 단일 화면·API | **메인 직접** (TDD + verify) | 없음 — 항상 직접 |
| 다관점 리뷰 / 검증 패널 / 리서치 fan-out (분 단위) | **Workflow** (in-process 병렬, 도구 가용 시) | 릴리즈 전 최종 검수는 Council 팀 (cross-provider) |
| 풀스택 피처 (DB→API→앱) | contracts 작성 → 메인 직접 순차 | 스택별 작업이 각 30분+ 이면 forge-team (장기 병렬 워커) |
| 리뷰/보안 검수 | 메인 직접 또는 Workflow 패널 | 릴리즈 전 최종 = Council (claude+gpt 적대 검수) |
| 큰 앱 부트스트랩 | Phase 0 인프라 결정 → 직접 | 독립 스택 3개 동시 부트스트랩이면 forge-team |
| 대량 마이그레이션 / 긴 테스트 매트릭스 | — | forge-team 백그라운드 잡 (메인은 다음 작업) |
| 설계 큰 결정 | 메인 초안 | 갈림길이 크면 Council 1회 |

forge-team 의 존재 이유 = **cross-provider 검수 · 관전/개입 · 세션 독립 장기 잡**.
짧은 fan-out 을 팀으로 하면 조정비용 (부팅+inbox+merge) 이 이득을 잡아먹는다.
