...
# Project: API Server Harness

**NestJS + Prisma + PostgreSQL** 백엔드 단독 레포. 프론트엔드 없음 —
외부 클라이언트 (앱/웹/서드파티) 가 소비하는 API 서버.

```
<project>/
├── src/
│   ├── auth/        → Passport + JWT (strategies, guards)
│   ├── <domain>/    → 도메인 모듈 (controller + service + dto)
│   ├── common/      → guards, interceptors, filters, pipes
│   └── prisma/      → PrismaService (싱글턴)
├── prisma/          → schema.prisma + migrations
├── test/            → Jest unit + supertest e2e
├── contracts/       → API 계약 (contract-first — OpenAPI 의 원본)
└── docs/
```

단일 레포 — git subtree 분리 없음 (git-workflow.md 의 subtree 섹션은 무시).
주 구현 멤버 agentId: `nestjs-backend`, `prisma-data`.
