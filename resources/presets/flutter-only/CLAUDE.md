...
# Project: Flutter App Harness

**Flutter + Riverpod + go_router** 앱 단독 레포. 자체 백엔드 없음 —
로컬 우선 (drift/secure storage) 또는 **Supabase BaaS** (인증/DB/스토리지).

```
<project>/
├── lib/
│   ├── core/           → config, network, theme, logger, utils
│   ├── data/           → remote (supabase/외부 API), local (drift/prefs), repository 구현
│   ├── domain/         → entity, repository 추상, usecase (순수 Dart)
│   └── presentation/   → 화면별 디렉터리 (위젯 + 컨트롤러)
├── test/  integration_test/
├── contracts/          → 외부 API / Supabase 테이블 계약 (contract-first)
└── docs/
```

단일 레포 — git subtree 분리 없음 (git-workflow.md 의 subtree 섹션은 무시).
주 구현 멤버 agentId: `flutter-ui`, `riverpod-logic`. BaaS 작업 시
`supabase-flutter` 스킬을 Read 하고 따른다.
