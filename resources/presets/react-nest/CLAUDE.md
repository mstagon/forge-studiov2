...
# Project: React + NestJS Harness

**React 19 + Vite 프론트 + NestJS + Prisma + PostgreSQL 백엔드** 풀스택.
계약(contracts/)을 원본으로 양쪽 DTO 동기화 (contract-first).

```
<project>/
├── client/       → React + Vite (TanStack Query + Router, Tailwind + shadcn/ui)
│   ├── src/features/    → 기능별 (components + hooks + api)
│   ├── src/shared/      → 공용 UI/유틸/api client
│   └── src/app/         → 라우팅, 전역 provider
├── server/       → NestJS (controller/service/dto + Prisma)
│   ├── src/<domain>/    → 도메인 모듈
│   ├── src/auth/        → Passport + JWT
│   └── src/prisma/      → PrismaService
├── prisma/       → schema.prisma + migrations
├── contracts/    → API 계약 (원본 — server DTO ↔ client 타입 동기화)
└── docs/
```

원격은 git subtree 로 client/server 분리 (`origin-app` / `origin-server`).
주 구현 멤버 agentId: `nestjs-backend`, `prisma-data`, `react-ui`(= flutter-ui 슬롯
대체 — 이 프리셋에선 React 담당). 프론트 데이터 패칭은 `react-query-patterns` 스킬 준수.
