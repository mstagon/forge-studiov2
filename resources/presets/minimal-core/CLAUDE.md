...
# Project: Minimal Core Harness

**스택 무관 코어 하네스** — 팀 (forge-team) / 안전 훅 / 검증 루프 / 학습 루프만.
스택별 룰·스킬·에이전트 없음. 어떤 언어/프레임워크 코드베이스에든 얹는다.

```
<project>/         → 기존 코드베이스 그대로
├── contracts/     → 모듈 간 계약 (선택 — contract-first 권장)
└── docs/          → lessons-learned 등
```

시작하면 먼저: 코드베이스를 훑고 `.claude/contexts/tech-stack.md` 에 실제
스택/빌드/테스트 명령을 채워라 — 이후 세션이 그걸 기준으로 일한다.
크로스 멤버 agentId 만 제공: planner / tech-architect / code-reviewer /
security-auditor / spec-verifier / test-writer / tdd-guide / doc-updater /
docs-lookup / refactor-cleaner / build-error-resolver / loop-operator / harness-optimizer.
