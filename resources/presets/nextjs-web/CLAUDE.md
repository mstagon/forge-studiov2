...
# Project: Next.js Web Harness

**Next.js 15 (App Router) + Prisma + Tailwind + shadcn/ui** 단일 레포 풀스택 웹.
별도 API 서버 없음 — Server Actions / Route Handlers 가 백엔드.

```
<project>/
├── app/          → 라우트 (Server Components 기본)
├── components/   → UI (shadcn/ui 우선)
├── lib/          → 서버 로직, prisma client, 유틸
├── prisma/       → schema.prisma + migrations
├── contracts/    → Server Action / API 계약 (contract-first)
└── docs/
```

단일 레포 — git subtree 분리 없음 (git-workflow.md 의 subtree 섹션은 무시).
주 구현 멤버 agentId: `nextjs-cms` (Next.js 전체 담당), `prisma-data`.
